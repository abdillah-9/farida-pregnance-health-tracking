...
import { useCreateChildcare } from './childcareHooks/useCreateChildcare'
...
import { useDeleteFormData } from './childcareHooks/useDeleteChildcare'
import { useUpdateFormData } from './childcareHooks/useUpdateChildcare'
...
export default function page() {
  //Using React Query to fetch data from supabase
    const {insertDataMutation} = useCreateChildcare();
  ...
  const {isLoading: ChildcareLoading, data: Childcare, error} =  useQuery({
  ...
  useEffect(()=>{
    if(Childcare && user){
      let ChildcareData = Childcare.filter((row)=>row.userID == user.id)
      setFetched({ChildcareData});
    }
  },[Childcare, user])
  ...
  if(!user || ChildcareLoading){
    return <LoadingSpinner/>
  }
  if(fetched.ChildcareData.length == 0 ){
    return (
      ...
      <Container containerStyle={{width:"100%", display:"flex", justifyContent:"space-between"}}>
        <Texts textStyle={headingStyle}>Childcare</Texts>
            ...
            Unconfirmed
              ...
              <Container containerStyle={{width:"100%"}}>
                <Button buttonStyle={createButton} actionHandler={showFormHandler}>
                  <Icon><BiAddToQueue /></Icon>New Entry
                </Button>
              </Container>
              <Form Childcare={fetched.ChildcareData} user={user} insertDataMutation={insertDataMutation}
  ...
  function sortButtonHandler(sortByString){
    ...
    if(sortByString == "all"){
      ...
      ChildcareData = Childcare.filter((row)=>row.userID == user.id)
    }

    else if(sortByString == "confirmed"){
      setSortState(sortByString)
      ChildcareData = Childcare.filter((row)=>row.userID == user.id && row.status == "confirmed")
    }
    else if(sortByString == "unConfirmed"){
      setSortState(sortByString)
      ChildcareData = Childcare.filter((row)=>row.userID == user.id && row.status == "unConfirmed")
    }
  ...
  function next(){
    if(pageNumber < (fetched.ChildcareData.length / pageRows)){
      setPageNumber(pageNumber +1)
    }
  }

  return (
    <div style={ChildcareContainer}>
      <Form Childcare={fetched.ChildcareData} user={user} insertDataMutation={insertDataMutation}
       updateDataMutation={updateDataMutation}/>
      <DeletePrompt mutateDeleting={mutateDeleting}/>
      <Container>
      ...
      <Container containerStyle={tableContainer}>
        <Table 
          Childcare={fetched.ChildcareData} 
          user={user} 
          sortState={sortState}
        ...
        <Pagination>
          <Pagination.Desc>
            {console.log(JSON.stringify(fetched.ChildcareData))}
            <div>
              <span>Showing </span> {(pageNumber - 1)*pageRows + 1} 
              <span> to </span> {Math.min(pageNumber * pageRows, fetched.ChildcareData.length)} 
              <span> results of </span> {fetched.ChildcareData.length}
            </div>
          </Pagination.Desc>
