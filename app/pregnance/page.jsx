...
import { useCreatePregnance } from './pregnanceHooks/useCreatePregnance'
...
import { useDeleteFormData } from './pregnanceHooks/useDeletePregnance'
import { useUpdateFormData } from './pregnanceHooks/useUpdatePregnance'
...
export default function page() {
  //Using React Query to fetch data from supabase
    const {insertDataMutation} = useCreatePregnance();
  ...
  const {isLoading: PregnanceLoading, data: Pregnance, error} =  useQuery({
    queryKey: ['pregnance'],
  ...
  useEffect(()=>{
    if(Pregnance && user){
      let PregnanceData = Pregnance.filter((row)=>row.userID == user.id)
      setFetched({PregnanceData});
    }
  },[Pregnance, user])
  ...
  if(!user || PregnanceLoading){
    return <LoadingSpinner/>
  }
  if(fetched.PregnanceData.length == 0 ){
    return (
      ...
      <Container containerStyle={{width:"100%", display:"flex", 
        ...
        <Texts textStyle={headingStyle}>Pregnancy</Texts>
            ...
            Unconfirmed
              ...
              <Container containerStyle={{width:"100%"}}>
                <Button buttonStyle={createButton} actionHandler={showFormHandler}>
                  <Icon><BiAddToQueue /></Icon>New pregnance data
                </Button>
              </Container>
              <Form Pregnance={fetched.PregnanceData} user={user} insertDataMutation={insertDataMutation}
  ...
  function sortButtonHandler(sortByString){
    ...
    if(sortByString == "all"){
      ...
      PregnanceData = Pregnance.filter((row)=>row.userID == user.id)
    }

    else if(sortByString == "confirmed"){
      setSortState(sortByString)
      PregnanceData = Pregnance.filter((row)=>row.userID == user.id && row.status == "confirmed")
    }
    else if(sortByString == "unConfirmed"){
      setSortState(sortByString)
      PregnanceData = Pregnance.filter((row)=>row.userID == user.id && row.status == "unConfirmed")
    }
  ...
  function next(){
    if(pageNumber < (fetched.PregnanceData.length / pageRows)){
      setPageNumber(pageNumber +1)
    }
  }

  return (
    <div style={PregnanceContainer}>
      <Form Pregnance={fetched.PregnanceData} user={user} insertDataMutation={insertDataMutation}
       updateDataMutation={updateDataMutation}/>
      <DeletePrompt mutateDeleting={mutateDeleting}/>
      <Container containerStyle={{width:"100%", display:"flex", 
      ...
      <Container containerStyle={tableContainer}>
        <Table 
          pregnancyData={fetched.PregnanceData} 
          user={user} 
          sortState={sortState}
        ...
        <Pagination>
          <Pagination.Desc>
            {console.log(JSON.stringify(fetched.PregnanceData))}
            <div>
              <span>Showing </span> {(pageNumber - 1)*pageRows + 1} 
              <span> to </span> {Math.min(pageNumber * pageRows, fetched.PregnanceData.length)} 
              <span> results of </span> {fetched.PregnanceData.length}
            </div>
          </Pagination.Desc>
      ...
      <Container>
        <Button buttonStyle={createButton} actionHandler={showFormHandler}>
          <Icon><BiAddToQueue /></Icon>Add pregnance data
        </Button>
      </Container>  
    </div>
  )
